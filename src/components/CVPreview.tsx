...
export function CVPreview({ cvData, brandingData }: CVPreviewProps) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden">
      ...
      >
        {/* Header */}
        <div className="mb-8 pb-6 border-b-2" style={{ borderColor: brandingData.primaryColor }}>
            ...
            <div className="flex-1">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-slate-600">
                  <Mail className="h-4 w-4" style={{ color: brandingData.secondaryColor }} />
                  <span>{cvData.personalInfo.email}</span>
                </div>
                {cvData.personalInfo.dateOfBirth && (
                  <div className="flex items-center gap-2 text-slate-600">
                    <Cake className="h-4 w-4" style={{ color: brandingData.secondaryColor }} />
                    <span>{new Date(cvData.personalInfo.dateOfBirth).toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit', year: 'numeric' })}</span>
                  </div>
                )}
                <div className="flex items-center gap-2 text-slate-600">
                  <Phone className="h-4 w-4" style={{ color: brandingData.secondaryColor }} />
                  <span>{cvData.personalInfo.phone}</span>
                </div>
                <div className="flex items-center gap-2 text-slate-600">
                  <MapPin className="h-4 w-4" style={{ color: brandingData.secondaryColor }} />
                  <span>{cvData.personalInfo.location}</span>
                </div>
                {cvData.personalInfo.linkedin && (
                  <div className="flex items-center gap-2 text-slate-600">
                    <Linkedin className="h-4 w-4" style={{ color: brandingData.secondaryColor }} />
                    <span>{cvData.personalInfo.linkedin}</span>
                  </div>
                )}
                {cvData.personalInfo.website && (
                  <div className="flex items-center gap-2 text-slate-600">
                    <Globe className="h-4 w-4" style={{ color: brandingData.secondaryColor }} />
          ...
          <div>
            ...
            <div className="space-y-6">
              {cvData.experiences.map((exp) => (
                <div key={exp.id} className="relative pl-6 border-l-2" style={{ borderColor: brandingData.primaryColor }}>
                  <div 
                  ...
                  <div className="mb-2">
                    ...
                    <div className="flex items-center gap-3 text-slate-600 mb-1">
                      <span className="font-medium" style={{ color: brandingData.secondaryColor }}>
                        {exp.company}
                      </span>
                      {exp.location && (
